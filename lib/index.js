...
var helper = {
  ...
  parallel: function parallel(promiseGens, concurrency) {
    ...
    var counter = 0
    var round = Math.floor(length / concurrency) + (length % concurrency === 0 ? 0 : 1)
    var promise
    var result = []
    while (counter < round) {
      (function () {
        var start = counter * concurrency
        var genPromiseItem = function () {
          var promises = promiseGens.slice(start, start + concurrency).map(function (promiseGen) {
            return promiseGen()
          })
          return helper.all(promises).then(function (data) {
            result = result.concat(data)
          })
        }
        if (!promise) {
          promise = genPromiseItem()
        } else {
          promise = promise.then(genPromiseItem)
        }
        counter++
      })()
    }
